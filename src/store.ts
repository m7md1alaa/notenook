...
export const visiblePageRangeAtom = atom((get) => {
  ...
  const buffer = Math.max(bottom - top, 1) * 0.75;
...
/** All PDFs loaded in the current session. */
export const pdfListAtom = atom<PdfEntry[]>([]);

/** The id of the PDF currently being viewed. */
export const activePdfIdAtom = atom<string | null>(null);
